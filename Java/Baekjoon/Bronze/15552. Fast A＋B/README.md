...
<p>본격적으로 for문 문제를 풀기 전에 주의해야 할 점이 있다. 입Output 방식이 느리면 여러 줄을 Input받거나 Output할 때 Time초과가 날 수 있다는 점이다.</p>

<p>C++을 사용하고 있고 <code>cin</code>/<code>cout</code>을 사용하고자 한다면, <code>cin.tie(NULL)</code>과 <code>sync_with_stdio(false)</code>를 둘 다 적용해 주고, <code>endl</code> 대신 개행문자(<code>\n</code>)를 쓰자. 단, 이렇게 하면 더 이상 <code>scanf</code>/<code>printf</code>/<code>puts</code>/<code>getchar</code>/<code>putchar</code> 등 C의 입Output 방식을 사용하면 안 된다.</p>
...
<p>Python을 사용하고 있다면, <code>input</code> 대신 <code>sys.stdin.readline</code>을 사용할 수 있다. 단, 이때는 맨 끝의 개행문자까지 같이 Input받기 때문에 String을 저장하고 싶을 경우 <code>.rstrip()</code>을 추가로 해 주는 것이 좋다.</p>

<p>또한 Input과 Output 스트림은 별개이므로, 테스트케이스를 전부 Input받아서 저장한 뒤 전부 Output할 필요는 없다. 테스트케이스를 하나 받은 뒤 하나 Output해도 된다.</p>
...
### Output 
 ...
 <p>각 테스트케이스마다 A+B를 한 줄에 하나씩 순서대로 Output한다.</p>
