...
<p>두 번째 줄부터 $N$개의 줄에 걸쳐 $(Q_i, A_i$)를 나타내는 String $Q_i$와 $A_i$가 공백으로 구분되어 주어진다. ($1 \leq |Q_i|, |A_i| \leq 10$; $i \neq j$이면 $Q_i \neq Q_j$)</p>

<p>그다음 줄부터 $M$개의 줄에 걸쳐 String $S_i$가 주어진다. ($1 \leq |S| \leq 100$)</p>

<p>Input되는 모든 String은 영어 대문자로만 이루어져 있고, 공백은 없다.</p>

### Output 

 <p>각 String $S_i$에 대해 당신이 만든 프로그램이 답해야 하는 String을 한 줄에 하나씩 Output하라.</p>

<p>만약 프로그램이 답할 수 있는 String이 없다면 <span style="color:#e74c3c;"><code>-1</code></span>을 대신 Output하라.</p>
